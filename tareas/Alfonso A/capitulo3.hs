...
> (!!!):: Int -> [a] -> Maybe a
> (!!!) x [] = Nothing
> (!!!) n (x:xs) = if n /= 0 then (!!!) (n-1) xs else Just x
...
> f6 :: Eq a => a->[a]->Int
> f6 t [] =  0
> f6 t (x:xs) = if t == x then 1+(f6 t xs) else f6 t xs
...
--f10 (headuno:uno) (headdos:dos) = (\i -> if headdos==headuno then i else i+1(f10 (headuno:uno) (dos)))

-- f10 (a:as) (b:bs) = if elem bs as then Just 1 else Nothing
...
> f14 :: Eq a => a -> [a] -> Bool
> f14 e (x:xs) = if e == x then True else f14 e xs
> f14 e [] = False
...
> isSubset :: Eq a => [a] -> [a] -> Bool
> isSubset [] [] = True
> isSubset (x:xs) bs = elem x bs && isSubset xs bs
...
--fractionPart (n:ns) = foldr (if n == '.' then n:fractionPart ns else n:[]) ns

Run all the tests
-----------------

> tests = test (t1++t3++t5++t7++t9++t11 ++t13 ++ t15 ++ t17 ++ t19 ++t2 ++ t3 ++ t4 ++ t5 ++ t6 ++ t7 ++ t8 ++ t9++t10++t11++t12++t13++t14 ++ t15 ++ t16 ++ t17 ++ t20)