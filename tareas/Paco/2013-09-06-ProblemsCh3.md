...
> f6 :: Eq a => a ->[a]->Int
> f6 t [] =  0
> f6 t (x:xs) = if x == t then 1+(f6 t xs) else f6 t xs
...
> f10 a b = search a b 0
> search::String->String->Int->Maybe Int 
> search [] needle c = Nothing 
> search (x:xs) needle c = if length needle > length (x:xs) 
>       	      then Nothing 
>		      else if take (length needle) (x:xs) == needle 
>		      	   then Just c 
>			   else search xs needle (c+1)
...
> isSubset :: Eq a => [a] -> [a] -> Bool
> isSubset [] a = True
> isSubset (a:as) x = elem a x && isSubset as x
...
> wholePart [] = []
> wholePart (x:xs) = if x == '.' then [] else x:wholePart xs
...
> 	  ++t2++t4++t6++t8++t10++t12++t14++t16++t20)
